...
class ApplicationController extends Controller {
	public function lastUploadedFilesAction() {
		...
		if ($query) {
			$content = $this -> container -> get('templating') -> render('SpookyIslandUploadBundle:Element:lastuploadedfiles.html.twig', array('gb2' => json_decode($gb, true), 'path' => $_SERVER['SCRIPT_NAME'] . '/../uploads/'));
			return new Response($content);
		}
		return new Response("no uploaded files");
	}
	...
	public function findAction() {
		...
		if (is_numeric(str_replace('-', '', $fname))) {
			...
			if ($query) {
				$contentFdTk = $this -> container -> get('templating') -> render('SpookyIslandUploadBundle:Element:finder.html.twig', array('foundtk2' => json_decode($foundtk, true), 'path2' => $_SERVER['SCRIPT_NAME'] . '/../uploads/'));
